...
export default function TabListaComparacao({ imoveisSalvos, onExcluir, salario, totalGastosFixos, onIniciarEdicao }: any) {
  ...
  const calcularSobra = (imovel: any) => {
    const custo = Number(imovel.aluguel || 0) + Number(imovel.condominio || 0) + Number(imovel.iptu || 0) + Number(imovel.luz || 0) + Number(imovel.agua || 0) + Number(imovel.gas || 0);
    ...
    <div className="space-y-6 pb-28">
      <div className="px-4 flex justify-between items-center">
        ...
        {selecionadosParaComparar.length > 0 && (
          <button onClick={() => setSelecionadosParaComparar([])} className="text-[9px] bg-purple-500/20 text-purple-400 px-4 py-1.5 rounded-full font-black border border-purple-500/30 uppercase animate-pulse">Duelo ({selecionadosParaComparar.length}/2)</button>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 px-2">
        {imoveisSalvos.map((imovel: any) => {
          ...
          return (
            <div key={imovel.id} onClick={() => setDetalhe(imovel)} className={`group bg-[#111114]/80 backdrop-blur-md border transition-all duration-300 rounded-[2.5rem] p-7 cursor-pointer hover:scale-[1.01] ${estaSelecionado ? 'border-purple-500 shadow-[0_0_40px_rgba(147,51,234,0.3)]' : 'border-white/5 hover:border-white/20'}`}>
              <div className="flex justify-between items-start mb-6 relative">
                ...
                <div className="flex gap-2 relative z-20" onClick={(e) => e.stopPropagation()}>
                  <button onClick={(e) => toggleComparacao(e, imovel)} title="Comparar" className={`p-3 rounded-2xl transition-all ${estaSelecionado ? 'bg-purple-600 text-white shadow-[0_0_15px_rgba(168,85,247,0.5)]' : 'bg-zinc-800/50 text-zinc-400 hover:text-purple-400'}`}>{estaSelecionado ? '✅' : '⚖️'}</button>
              ...
              <div className="bg-black/60 rounded-[1.8rem] p-5 border border-purple-900/20 mb-4 pointer-events-none">
                <div className="flex justify-between items-center mb-4"><span className="text-[9px] uppercase font-black text-zinc-600 tracking-widest">Custo Total</span><span className="text-lg font-bold text-rose-400 drop-shadow-[0_0_5px_rgba(251,113,133,0.3)]">{formatarMoeda(custo)}</span></div>
                <div className="flex flex-col items-end"><span className="text-[9px] uppercase font-black text-zinc-600 tracking-widest mb-1">Sobra Final</span><span className={`text-2xl font-black tracking-tighter drop-shadow-[0_0_10px_rgba(168,85,247,0.3)] ${sobra >= 0 ? 'text-emerald-400' : 'text-rose-500'}`}>{formatarMoeda(sobra)}</span></div>
              </div>

              {/* LOGÍSTICA JF (CHAVES CORRIGIDAS AQUI!) */}
      ...
      {/* MODAL RAIO-X (CHAVES CORRIGIDAS NO PAI TAMBÉM) */}
      {detalhe && (
        <div className="fixed inset-0 z-[110] bg-black/95 backdrop-blur-md flex items-center justify-center p-4" onClick={() => setDetalhe(null)}>
          <div className="bg-[#0a0a0c] border border-purple-500/20 p-10 rounded-[3rem] max-w-sm w-full shadow-2xl animate-in zoom-in-95" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-8 border-b border-white/5 pb-4"><h2 className="text-xl font-black text-white uppercase italic drop-shadow-[0_0_10px_rgba(168,85,247,0.5)]">{detalhe.nome}</h2><button onClick={() => setDetalhe(null)} className="text-zinc-500 hover:text-white text-xl">✕</button></div>
            <div className="space-y-4 text-[10px] font-bold uppercase tracking-widest text-zinc-400">
              ...
              <div className="border-t border-white/5 pt-4 space-y-2">
                <div className="flex justify-between items-center text-zinc-600"><span className="text-[8px]">Percurso</span><span className="text-[8px]">Carro 🚗</span><span className="text-[8px]">A pé 🏃</span></div>
                {/* CORREÇÃO DO TEMPO DO PAI AQUI! */}
            ...
            <button onClick={() => setDetalhe(null)} className="w-full mt-10 bg-zinc-900 p-5 rounded-3xl font-black text-zinc-500 uppercase tracking-widest border border-white/5 hover:bg-zinc-800 transition-colors">Fechar</button>
          </div>
        </div>
      )}
    </div>
